...
import {
    B2C_SHOPPING_CART,
    ENTITLEMENTS_VALIDATE,
} from "../../../utils/acmeEndpoints";
import {
    EntitlementValidationPayload,
    ShoppingCartObject,
 ...
 */
export async function createNewShoppingCart(
    cart: {} | ShoppingCartObject
): Promise<string> {
    const payload = (await performRequest({
        url: B2C_SHOPPING_CART,
        method: "post",
        data: cart,
    })) as string;
    return payload;
 ...
 */
export async function getExistingShoppingCart(
    id: string
): Promise<ShoppingCartObject> {
    const url = `${B2C_SHOPPING_CART}/${id}`;

    const payload = (await performRequest({
        url,
        method: "get",
    })) as ShoppingCartObject;
    return payload;
}
 ...
 */
export async function updateExistingShoppingCart(
    id: string,
    cart: ShoppingCartObject
): Promise<ShoppingCartObject> {
    const url = `${B2C_SHOPPING_CART}/${id}`;
 ...
 */
export async function deleteExistingShoppingCart(
    id: string
): Promise<ShoppingCartObject> {
    const url = `${B2C_SHOPPING_CART}/${id}`;
...
export async function validateShoppingCart(
    data: { id: string } | ShoppingCartObject
): Promise<EntitlementValidationPayload> {
    const payload = (await performRequest({
        url: ENTITLEMENTS_VALIDATE,
        method: "post",
        data,
    })) as EntitlementValidationPayload;
    return payload;
}
