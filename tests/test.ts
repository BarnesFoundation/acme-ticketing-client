...
import {
    TicketingFunctions as tfc,
    MembershipSaleFunctions,
} from "../convenience";
...
const definedReportFetchExample = async () => {
    ...
    const { queryExpression, dateSettings } =
        await ReportFunctions.getReportDefinition(uuid);
    const { endDate, startDate, endDateTime, startDateTime, dateRangeField } =
        dateSettings;
    const reportExecution = await ReportFunctions.executeReport({
        reportUuid: uuid,
        queryExpression,
        endDate: "2019-12-03T00:00:00-05:00",
        endDateTime: "2019-12-03T00:00:00-05:00",
        startDate: "2019-12-01T00:00:00-05:00",
        startDateTime: "2019-12-01T00:00:00-05:00",
        dateRangeField,
    });
    ...
    const pollUntilComplete = () => {
        return new Promise((resolve) => {
            const interval = setInterval(async () => {
                if (status !== "Completed") {
                    [
                        ({ status } = await ReportFunctions.pollForReportStatus(
                            id
                        )),
                    ];
    ...
    const reportResult = await ReportFunctions.retrieveReportResults(
        id,
        "json"
    );
...
const ordersForMembershipDateRangeExample = async (membershipId: string) => {
    const orderPayloads = await tfc.getOrdersForMembershipDateRange(
        membershipId,
        "2020-01-15T00:00:00.000-05:00",
        "2020-01-16T00:00:00.000-05:00"
    );
    console.log(JSON.stringify(orderPayloads));
};

const eventTemplateSummariesExample = async () => {
    const payload = await EventTemplateFunctionsB2C.listEventTemplateSummaries({
        saleChannel: "online",
    });
...
const eventActivityCalendarsExample = async (params) => {
    const payload =
        await EventTemplateFunctionsB2C.getActivityCalendarForTemplate(params);
    console.log(payload);
};
...
const eventTemplateTimesExample = async (params) => {
    ...
    const specifiedEventTimes =
        await EventTemplateFunctionsB2C.getTemplateTimes(params);
...
const shoppingCartB2CExample = async () => {
    ...
    const createdShoppingCartId =
        await ShoppingCartFunctionsB2C.createNewShoppingCart(
            populatedShoppingCart
        );
    console.log(
        `The id of the created shopping cart is ${createdShoppingCartId}`
    );

    const retrievedShoppingCart =
        await ShoppingCartFunctionsB2C.getExistingShoppingCart(
            createdShoppingCartId
        );
    console.log(
        `The shopping cart belonging to ${createdShoppingCartId} is`,
        retrievedShoppingCart
    );

    const shoppingCartValidation =
        await ShoppingCartFunctionsB2C.validateShoppingCart(
            populatedShoppingCart
        );
    console.log(
        `The validation of the on-the-fly shopping cart was`,
        shoppingCartValidation
    );

    const deletedShoppingCart =
        await ShoppingCartFunctionsB2C.deleteExistingShoppingCart(
            createdShoppingCartId
        );
    console.log(
        `The shopping cart with id ${createdShoppingCartId} was deleted and the response was`,
        deletedShoppingCart
    );
};
...
const listEventTemplatesB2CExample = async () => {
    ...
    const eventTemplates = await EventTemplateFunctionsB2C.listEventTemplates({
        slim: true,
        saleChannel: "online",
    });
...
const sendOrderEmailExample = async () => {
    ...
    const orderResponse = await CheckoutFunctionsB2B.sendOrderEmail(
        orderNumber,
        emailInput
    );
...
const readEventTemplateImageExample = async () => {
    ...
    const imageResponse = await EventImagesFunctions.readTemplateProfileImage({
        templateId,
        size,
    });
...
const membershipLevelsExample = async () => {
    ...
    const filteredMembershipLevels =
        await MembershipLevelsFunctions.listMembershipLevels({
            isPublished: true,
            salesChannels: "pointOfSale",
            summarize: true,
        });
    console.log(filteredMembershipLevels);

    // Get a specific membership level, by id
    const specificLevel = await MembershipLevelsFunctions.getMembershipLevel(
        "58b0a65c969c6d3155d6fe87"
    );
...
const listEventTemplateInstancesExample = async () => {
    const eventTemplateInstances =
        await EventTemplateFunctionsB2C.listEventTemplateInstances({
...
const placeAnOrder = async () => {
    try {
        ...
        const purchase = await ECommerceFunctionsB2C.performCheckout(
            billingInformationAndCart,
            uuid
        );
...
const placeAnOrderAndThrowError = async () => {
    try {
        ...
        await ECommerceFunctionsB2C.performCheckout(
            billingInformationAndCart,
            uuid,
            { throwRaw: true }
        );
...
const updateEventCapacity = async () => {
    ...
    const fetchedEvent = await EventFunctions.getEvent(
        "600602634791d654027aa6d3"
    );
    ...
    console.log(
        `Both events now have event capacity of 37: ${
            updatedEvent.capacity === fetchedEvent.capacity
        }`
    );
};
...
const performMembershipPurchase = async () => {
    ...
    const newMembershipOrder =
        await MembershipSaleFunctions.purchaseNewMembership(
...
const renewMembership = async () => {
    const membershipActionResponse =
        await MembershipSaleFunctions.processMembershipAction(
            {
...
const doFormFunctions = async () => {
    // Get a paginated list of form responses
    const listOfFormResponses = await FormFunctions.fetchFormResponses({
        page: 1,
        pageSize: 20,
    });

    // Use the id of the first form to retrieve its individual response
    if (listOfFormResponses.list.length > 0) {
        const singleFormResponse = await FormFunctions.fetchFormResponse(
            listOfFormResponses.list[0].id
        );

        console.log(singleFormResponse);
    }
